...
function getDiceComment(rolls, count, faces) { const total = rolls.reduce((a, b) => a + b, 0); const M = count * (faces + 1) / 2; const sigma = Math.sqrt(count * (faces * faces - 1) / 12); const lower = M - 0.6745 * sigma; const upper = M + 0.6745 * sigma; let comment = total > upper ? 'すごいです！' : total < lower ? 'えっと、次がありますよ…！' : 'いい感じです！'; return `サイコロ振りますねー！\n[${rolls.join(', ')}]\n${comment}`; }
